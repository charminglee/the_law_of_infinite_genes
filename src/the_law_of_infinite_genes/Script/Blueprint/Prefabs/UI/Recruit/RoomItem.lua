...
function RoomItem:SetItemInfo(ItemData)
    self:Clear();
...
---耐久值越低，遮罩覆盖面积越大。
function RoomItem:UpdateDurabilityMask()
    -- 耐久遮罩
...
function RoomItem:UpdateAttachInfo()
	-- 根据品质排序
...
function RoomItem:AttachUpdateItem(widget, idx)
...
function RoomItem:OnCustomUICreated(CustomUI)
...
function RoomItem:SetCustomUISoftWidgetPath(SoftWidgetPaths, PostCallback)
    -- 清空自定义UI
    self.CanvasPanel_0:ClearChildren()
...
function RoomItem:SetIsSelect(isSelect)
    if isSelect then
