...
function DegreeChoiceItem:Construct()
...
function DegreeChoiceItem:SetItemInfo(ItemData)
    self:Clear();
...
---耐久值越低，遮罩覆盖面积越大。
function DegreeChoiceItem:UpdateDurabilityMask()
    -- 耐久遮罩
...
function DegreeChoiceItem:UpdateAttachInfo()
	-- 根据品质排序
...
function DegreeChoiceItem:AttachUpdateItem(widget, idx)
...
function DegreeChoiceItem:OnCustomUICreated(CustomUI)
...
function DegreeChoiceItem:SetCustomUISoftWidgetPath(SoftWidgetPaths, PostCallback)
    -- 清空自定义UI
    self.CanvasPanel_0:ClearChildren()
...
function DegreeChoiceItem:GetCustomUISoftWidgetPath()
    return self.CutomeUISoftClassPath
end
-- 设置物品品质显示
-- 根据品质等级加载对应的品质背景和品质条纹理，nil时隐藏对应图片。
-- @param Quality number @物品品质等级
function DegreeChoiceItem:SetQuality(Quality)
...
function DegreeChoiceItem:SetIsSelect(isSelect)
    if isSelect then
