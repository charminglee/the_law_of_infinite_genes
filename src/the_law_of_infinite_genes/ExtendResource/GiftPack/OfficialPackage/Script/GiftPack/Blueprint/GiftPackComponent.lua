...
function GiftPackComponent:OnAddVirtualItem(Result)
    ...
    if bSucceeded and PlayerKey == SelfPlayerKey and RequestMark ~= self.RequestMark then
        local GiftPackDatas = GiftPackManager:GetGiftPackData();
        for ItemID, ItemNum in pairs(ItemList) do
            for GiftPackID, GiftPackData in pairs(GiftPackDatas) do
                if GiftPackData.ItemID == ItemID and
                    GiftPackData.GiftPackType == EGiftPackType.Normal and
                    GiftPackData.OpenWay == EGiftPackOpenType.AutoOpen then
                    GiftPackManager:OpenNormalGiftPackage(GiftPackID, ItemNum, PlayerController);
                    break;
                end
            end
        end
